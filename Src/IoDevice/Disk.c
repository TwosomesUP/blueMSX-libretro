...
static DiskOverlay* diskOverlays[MAXDRIVES] = {0};
...
UInt8 diskWriteSector(int driveId, UInt8 *buffer, int sector, int side, int track, int density)
{
    ...
    /* Overlay logic: always write to overlay */
    if (diskOverlays[driveId]) {
...
void mountDiskImage(int driveId, const char* dskImagePath, size_t sectorSize)
{
    if (diskOverlays[driveId])
    {
...
void unmountDiskImage(int driveId)
{
